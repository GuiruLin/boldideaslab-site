...
**顶栏形态（2026-06-12 二次修订，owner 反馈"太像 Streamtime"后重做）**：K12 教育站逻辑——信任先于转化，导航全貌可见。桌面：logo + 字标在左，六个问句横排在右，hover 时小金拱从字底升起（下划线是形状不是线）；"Shall we talk?" 是导航末位的普通问句，仅蓝色文字区分，不做按钮、不钉角。移动：Menu 药丸 + 问句瀑布（收纳是移动端通用模式）。

**滚动与含义动效（2026-06-12 新增）**：section 进入视口轻浮现（16px/500ms/一次）；三力卡 hover 时形状做"自己的动作"——十边缓转（思维在转）、金拱上升（作品在升）、pill 横向伸展（在表达）；金色大数字进入视口从 0 数到目标值（900ms）。
